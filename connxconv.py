...
model_name = ("wav2lip_gan" if len(argv) < 2 else argv[1]).replace('.pth', '')

model = Wav2Lip()

state = torch.load(f"{model_name}.pth", map_location=lambda storage, loc: storage)
...
# usage: python onnxconv.py [wav2lip|wav2lip_gan]